...
/self->object/
{
	self->object = vtimestamp; /* this should have been set in above object-create, here it will always be zero */
...
/self->gc/
{
	...
	self->gc = 0;
	@num[".", "gcsweep", "-"] = count();
	@types[".", "gcsweep", "-"] = sum(this->elapsed);
	self->exclude[self->depth] += this->elapsed;
}
